...
const converterEventEmitter =
    require('MTA_Subway_GTFS-Realtime_to_SIRI_Converter').ConverterEventEmitter

const loggingService = require(path.join(__dirname, '../logging/MockLoggingService'))
...
converterEventEmitter.on(
    converterEventEmitter.eventTypes.LOCATIONS_UPDATE,
    handleTrainLocationUpdateEvent
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.TRAIN_TRACKING_STATS_UPDATE,
    handleTrainTrackStatsUpdateEvent
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.UNSCHEDULED_TRIPS_UPDATE,
    handleUnscheduleTripsUpdateEvent
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.NO_SPATIAL_DATA_TRIPS_UPDATE,
    handleNoSpatialDataTripsUpdateEvent
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.TRAIN_TRACKING_ERROR,
    handleTrainTrackingError
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.DATA_ANOMALY,
    handleDataAnomaly
)

converterEventEmitter.on(
    converterEventEmitter.eventTypes.ERROR,
    handleError
)
