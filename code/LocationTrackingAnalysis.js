...
let trackingAll = false

const tripsWithCoordsLog = []
let trackingAllIndex = -1

const timestampToMessageNumber = {}
let messageCounter = 0

let trainHadLocationCount = 0
let lostTrainCount = 0
...
function analyzeTripsWithCoords () {
    ...
    let toFractions = tripsWithCoordsLog.slice(trackingAllIndex)
                                        .map(pair => (pair[0] / pair[1] * 100))
...
function analyzeTripAppearances () {
  const messageCountGapsBetweenAppearances = _.reduce(tripTrackingMap, (acc, locArrArr, key) => {

    const firstOccurrence = _.first(_.first(locArrArr))
    const lastOccurrence  = _.last(_.last(locArrArr))

    const locArrArrFiltered = locArrArr.filter(locArr => locArr.length)

    const startEndLocArrPairs = locArrArrFiltered.map(function (locArr) {
            let firstTimestamp = _.first(locArr).timestamp,
        ...
        gaps = _.tail(startEndLocArrPairs).map(function (pair, i) {
            ...
            if (!isNaN(gapEnd)) {
             gapRange.push({ msgNum: gapEnd, stopId: stopEnd, timestampStop: tsStop })
            }
    ...
    let byAppearanceCount = _.reduce(messageCountGapsBetweenAppearances, function (acc, gapPairArr, key) {
...
function analyzeDistancesTraveled () {
    ...
    let tripKeyToSpeedsArr =  _.reduce(tripTrackingMap, function (acc, locArrArr, key) {


            let locArr = _.flatten(locArrArr) ,
                ...
                speeds = _.tail(distTimestampPairs).map(function (pair, i) {
                    ...
                    let dX = (pair.d - distTimestampPairs[i].d) ,
 ...
 acc2.push(speed)
            ...
            if (trimDwellAtOrigin.length) {
 acc[key] = trimDwellAtOrigin
}
...
function feedListener (err, gtfsrtJSON, siriJSON, converterCache, cb) {
    ...
    //
    let trainTrackerSnapshot = converterCache.converter.trainTrackerSnapshot ,
        ...
        unschedMap = __unscheduled.reduce(function (acc, trip) {
                        acc[trip.split('_').slice(-2).join('_')] = 1
                        return acc
                     }, {}) ,
    ...
    _.forEach(vehicleActivity, function (vehAct) {
        let siriKey = vehAct.MonitoredVehicleJourney
        ...
        if (unschedMap[gtfsrtKey] || noDataMap[gtfsrtKey]) {
         return
        }
    ...
    (function () {
        _.forEach(trackedTrainGTFSrtKeys, function (key) {
            ...
            let gtfsKey = gtfsrtKeyToGTFSKey[key] ,
