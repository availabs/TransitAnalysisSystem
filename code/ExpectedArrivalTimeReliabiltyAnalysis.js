...
function feedListener (err, gtfsrtJSON, siriJSON, converterUpdate, cb) {
  if (err) {
  ...
  const vehicleActivity = siriJSON.Siri.ServiceDelivery.VehicleMonitoringDelivery[0].VehicleActivity
  ...
  vehicleActivity.forEach((vehAct) => {
      ...
      if ( (recordedAtTime !== recordedAtTimes[vehicleRef]) &&
           (previousPair && (previousPair.stop === nextStop))
      ) {
